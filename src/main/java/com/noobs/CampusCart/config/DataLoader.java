...
public class DataLoader implements CommandLineRunner {
    ...
    public void run(String... args) throws Exception {
        ...
        Product p1 = new Product("K-9 wireless microphone", 800.0, "Good", "/images/microphone.jpg", "sell", "approved", u1, c1, 10,
                2);
        ...
        List<Review> demoReviews = List.of(
                Review.builder().user(u1).rating(5).comment("CampusCart made buying books so easy!")
                        .reviewDate(LocalDateTime.now().minusDays(1)).build(),
                Review.builder().user(u2).rating(4)
                        .comment("Great platform, but can add more categories.")
                        .reviewDate(LocalDateTime.now().minusDays(2)).build(),
                Review.builder().user(u3).rating(3)
                        .comment("Decent, but sometimes items take time to approve.")
                        .reviewDate(LocalDateTime.now().minusDays(3)).build(),
                Review.builder().user(u4).rating(5)
                        .comment("Loved the experience, very smooth and user-friendly.")
                        .reviewDate(LocalDateTime.now().minusHours(10)).build(),
                Review.builder().user(u5).rating(2)
                        .comment("Had issues with login once, but got fixed.")
                        .reviewDate(LocalDateTime.now().minusDays(4)).build(),
                Review.builder().user(u6).rating(4)
                        .comment("Really helped me find hall essentials quickly.")
                        .reviewDate(LocalDateTime.now().minusHours(30)).build(),
                Review.builder().user(u7).rating(5).comment("Perfect app for CUETians!")
                        .reviewDate(LocalDateTime.now().minusDays(5)).build(),
                Review.builder().user(u8).rating(1).comment("Needs more improvement in search filters.")
                        .reviewDate(LocalDateTime.now().minusDays(6)).build(),
                Review.builder().user(u2).rating(5).comment("Awesome! Found my calculator instantly.")
                        .reviewDate(LocalDateTime.now().minusDays(7)).build(),
                Review.builder().user(u3).rating(4).comment("Easy to sell and buy items here.")
                        .reviewDate(LocalDateTime.now().minusDays(8)).build());

        reviewRepository.saveAll(demoReviews);
